...
our $exp_acc;
our($opt_F,$opt_R,$opt_I,$opt_O, $opt_P, $opt_S, $opt_D, $opt_A, $opt_V);

getopts('F:R:I:O:P:S:D:A:V:');
	...
	foreach my $result (@results){
		...
		if($all_idents{$conflict}){
			...
			if($prev_hit->evalue() == $eval && $prev_seq eq $pep_seq && $prev_hit->searchEngine() eq $search_engine){				
				...
				die "Falal error, different peptide ($pep_seq (e: vs $eval".$prev_hit->pepSeq() . $prev_hit->evalue() .") with same PSM number for search engine: $search_engine\n";
			}		
		}
		else{
...
sub combineAcrossSE{
	...
	for my $exp_title ( keys %exp_title_prot_cons ) {	
		...
		my ($mascot_tp01,$mascot_fp01) = printPeptideResults(\@mascot_idents,0.01,"Mascot");
		my ($mascot_tp05,$mascot_fp05) = printPeptideResults(\@mascot_idents,0.05,"Mascot");
		my ($omssa_tp01,$omssa_fp01) = printPeptideResults(\@omssa_idents,0.01,"Omssa");
		my ($omssa_tp05,$omssa_fp05) = printPeptideResults(\@omssa_idents,0.05,"Omssa");
		my ($tandem_tp01,$tandem_fp01) = printPeptideResults(\@tandem_idents,0.01,"X!Tandem");
